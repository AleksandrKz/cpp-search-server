...
public:
    ...
    {
        CheckSpecialCharacters(stop_words_);
    }
...
private:
    ...
    void CheckNewID(int id) {
        if (id < 0) {
            throw invalid_argument("Invalid ID.");
        }
        if (documents_.count(id) > 0) {
            throw invalid_argument("Document with given ID already exists.");
        }
    }
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty()) {
            throw invalid_argument("No text after character '-'.");
        }
        if (text[0] == '-') {
            throw invalid_argument("Having more than one minus before a word.");
        }
        if(!IsValidWord(text)) {
            throw invalid_argument("Word having invalid characters.");
        }
